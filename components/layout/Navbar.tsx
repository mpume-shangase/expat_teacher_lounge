...
export default function Navbar() {
  ...
  return (
          <motion.nav
                    className={scrolled ? 'fixed top-0 left-0 right-0 z-50 bg-white/95 dark:bg-[#0a0b14]/95 backdrop-blur-xl shadow-sm border-b border-black/8 dark:border-white/8' : 'fixed top-0 left-0 right-0 z-50 bg-transparent'}
                    initial={{ y: -100 }}
                    animate={{ y: 0, transition: { duration: 0.6 } }}
                  >
              {!scrolled && (
                              <div className="absolute inset-0 pointer-events-none" style={{ background: 'linear-gradient(to bottom, rgba(0,0,0,0.5) 0%, transparent 100%)' }} />
                            )}
